    ...
    for d, doc in tqdm(list(enumerate(documents))):
        ...
        for w in doc:
            pz = np.multiply(np.divide(category_topic[cat, :], topic_c + alpha_mod),
                             ...
                             np.divide(author_topic[auth, :], topic_c + eta_mod))
            z = np.random.multinomial(1, pz / pz.sum()).argmax()
            curr_doc.append(z)
            category_topic[cat, z] += 1
            author_topic[auth, z] += 1
            topic_word[z, w] += 1
            topic_c[z] += 1
        wt_assignment.append(curr_doc)
    return wt_assignment, category_topic, author_topic, topic_word, topic_c, alpha_mod, beta_mod, eta_mod


def gibbs_sampling(documents: List[np.ndarray],
                   cat_topic: np.ndarray,
                   auth_topic: np.ndarray,
    ...
    for d_index, doc in enumerate(documents):
        for w_index, word in enumerate(doc):
            ...
            decrease_count(topic, topic_word, cat_topic, auth_topic, c_index, a_index, word, topic_count)
            ...
            pz = np.multiply(np.divide(cat_topic[c_index, :], topic_count + alpha_mod),
                             ...
                             np.divide(auth_topic[a_index, :], topic_count + eta_mod))
            topic = np.random.multinomial(1, pz / pz.sum()).argmax()
            word_topic_assignment[d_index][w_index] = topic

            # And increase the topic count
            increase_count(topic, topic_word, cat_topic, auth_topic, c_index, a_index, word, topic_count)


def increase_count(topic, topic_word, cat_topic, auth_topic, d_index, a_index, word, t_count):
    cat_topic[d_index, topic] += 1
    auth_topic[a_index, topic] += 1
    topic_word[topic, word] += 1
    t_count[topic] += 1


def decrease_count(topic, topic_word, cat_topic, auth_topic, d_index, a_index, word, t_count):
    cat_topic[d_index, topic] -= 1
    auth_topic[a_index, topic] -= 1
    topic_word[topic, word] -= 1
    t_count[topic] -= 1
...
if __name__ == '__main__':
    ...
    word_topic_assignment, category_topic_dist, author_topic_dist, topic_word_dist, topic_count, alpha_mod, beta_mod, eta_mod = random_initialize(train_docs)
    ...
    for i in tqdm(range(0, iterationNum)):
        gibbs_sampling(train_docs, category_topic_dist, author_topic_dist, topic_word_dist, topic_count, word_topic_assignment)
