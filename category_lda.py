...
def model(doc_word_data=None, category_data=None, num_words_per_doc=None, args=None, batch_size=None):
    # Globals.
...
def parametrized_guide(doc_word_data, category_data, num_words_per_doc, args, batch_size=None):
...
def main(args):
    ...
    for step in tqdm(range(args.num_steps)):
        loss = svi.step(doc_word_data=doc_word_data, category_data=category_data, num_words_per_doc=num_words_per_doc,
                        args=args, batch_size=args.batch_size)
        if step % 10 == 0:
            logging.info('{: >5d}\t{}'.format(step, loss))
    loss = elbo.loss(model, parametrized_guide, doc_word_data=doc_word_data, category_data=category_data,
                     num_words_per_doc=num_words_per_doc, args=args)
